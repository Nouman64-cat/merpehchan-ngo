...
export default function AdminEventsListPage() {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      setEvents((current) => current?.filter((e) => e._id !== id) ?? null);
    } catch (err) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-soft">
            Manage the events shown on the public Gallery page.
          </p>
        </div>
        <Link
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-black/5 bg-white">
        {events === null ? (
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {events.map((event) => (
                <tr key={event._id} className="border-b border-black/5 last:border-0">
                  ...
                  <td className="px-6 py-4 text-ink-soft">{event.date}</td>
                  <td className="px-6 py-4 text-ink-soft">{event.photos.length}</td>
                  <td className="px-6 py-4 text-ink-soft">{event.display_order}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    ...
                    >
                      {deletingId === event._id ? "Removing…" : "Delete"}
                    </button>
                  </td>
                </tr>
              ))}
